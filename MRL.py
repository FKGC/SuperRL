...
class FRL(nn.Module):
    ...
    def forward(self, support, support_meta,
        ...
        if is_train:
            ...
            support_rep = self.entity_encoder(support, support_meta)# (5,200) support(5,2) 
            query_rep = self.entity_encoder(query, query_meta)# (512,200)
            false_rep = self.entity_encoder(false, false_meta)# (512,200)
